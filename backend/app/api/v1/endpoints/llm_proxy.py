...
async def llm_proxy(
    ...
    is_developer = getattr(current_user, "is_developer", False) and getattr(
        current_user, "dev_plan_active", False
    )
    ...
    if not is_developer:
        ...
        if key is not None:
            raise HTTPException(
                status_code=429, detail=f"Límite de uso excedido ({key}: {limit})"
            )
    ...
    if files:
        for f in files:
            ...
            if mime.startswith("image/"):
                ...
                message_content.append({"type": "audio", "audio": {"url": data_url}})
            else:
    ...
    if getattr(settings, "LOG_LEVEL", "").upper() == "DEBUG" and "localhost" in str(
        getattr(settings, "SERVER_HOST", "")
    ):
        logging.getLogger("llm_proxy").debug(
            f"[DEBUG][localhost] Mensaje enviado a LLM: {messages}"
        )
        logging.getLogger("llm_proxy").debug(
            f"[DEBUG][localhost] Respuesta cruda del LLM: {llm_output}"
        )
    return JSONResponse(content={"llm_output": llm_output})

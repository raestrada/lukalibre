#!/usr/bin/env python3
"""
Test script for LLM Proxy API endpoints with demo user authentication
Creates a demo user and tests specialized models
"""
...
def authenticate_demo_user():
    ...
    login_data = {
        "username": "admin@lukalibre.org",  # FastAPI OAuth2 uses 'username' field
        "password": "admin123"
    }
...
def test_token_validity(access_token):
    ...
    try:
        response = requests.post(
            f"{API_V1}/auth/login/test-token",
            headers=headers,
            timeout=10
        )
...
def main():
    """Run all demo user authenticated API tests"""
    print("🚀 Starting Demo User Authenticated LLM Proxy Tests")
    print("=" * 60)
    ...
    # Skip demo user creation, use existing admin user
    print("💡 Using existing admin user for testing")
    
    # Authenticate with demo user
    access_token = authenticate_demo_user()
    if not access_token:

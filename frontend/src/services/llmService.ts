...
function shouldUseLocalProxy(): boolean {
  ...
  const hasPlan =
    plan &&
    plan.is_active &&
    (plan.credits > 0 || ((plan.developer || plan.is_developer) && plan.dev_plan_active));
...
export async function extractAndInsertData(
  ...
  try {
    ...
    if (typeof llmResp === 'string' && llmResp.toLowerCase().includes('sorry')) {
      throw new Error(
        `OpenAI rechazó la solicitud con: ${llmResp.substring(0, 100)}...\n\n` +
  ...
  if (!parsed.sql_inserts)
    throw new Error('No se encontraron comandos SQL en la respuesta del LLM.');

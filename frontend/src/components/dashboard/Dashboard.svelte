    ...
    <div class="llm-bar">
      <form class="llm-bar-flex" on:submit={handleLLMProxySubmit}>
        ...
        <input
          ...
          type="file"
          accept="image/*,.pdf"
          on:change={(e) => (llmFile = (e.target as HTMLInputElement)?.files?.[0] ?? null)}
        />
        <Button
          type="submit"
          variant="primary"
      ...
      {/if}
      {#if llmSuccess}
        <StatusMessage type="success" message={llmSuccess} onClose={() => (llmSuccess = null)} />
      {/if}
      {#if llmError}
        <StatusMessage type="error" message={llmError} onClose={() => (llmError = null)} />
      {/if}
    </div>
    <DashboardReport />
    {#if !dbInitialized}

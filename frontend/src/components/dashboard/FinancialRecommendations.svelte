<script lang="ts">
  ...
  async function generateRecommendations() {
    ...
    try {
      ...
      localStorage.setItem('financial_recommendations', JSON.stringify({
  ...
  function getPriorityIcon(priority: string): string {
    switch(priority.toLowerCase()) {
      case 'alta': return 'alert-triangle';
  ...
  {:else if recommendations}
    <div class="financial-score">
      ...
      <ProgressBar 
        ...
        variant={
          recommendations.puntaje_salud_financiera >= 7 ? 'success' :
          recommendations.puntaje_salud_financiera >= 4 ? 'warning' : 'error'
        }
      />
    ...
    <div class="recommendations-list">
              ...
              <div class="recommendation-content" transition:slide={{duration: 200}}>
                ...
                {#if rec.accionable !== undefined}
                  <div class="recommendation-actionable">
                    ...
                    {rec.accionable ? 'Puedes implementarlo de inmediato' : 'Requiere planificación'}
                  </div>
                {/if}
              </div>
            {/if}
      ...
      {:else}
        <div class="no-recommendations">
          ...
          <p>No hay recomendaciones disponibles. Añade más transacciones para obtener recomendaciones personalizadas.</p>
        </div>
      {/if}
    </div>
  {:else}
...
<style>
  ...
  .recommendation-impact, .recommendation-actionable {
    font-size: 0.9rem;
  ...
  .no-recommendations p, .no-data p {
    margin: 1rem 0;
    max-width: 400px;
  }

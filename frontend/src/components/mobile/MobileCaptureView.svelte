<script lang="ts">
  ...
  async function setupCamera() {
    try {
      ...
      hasCamera = false;
      statusMessage = 'No se pudo acceder a la cámara. Por favor, revisa los permisos del navegador o sube una imagen manualmente.';
      statusType = 'error';
    }
  }
    ...
    try {
      ...
      reader.onloadend = async function() {
        ...
        try {
          ...
          statusMessage = '¡Recibo guardado exitosamente! Puedes seguir capturando más boletas o revisar los datos capturados.';
          statusType = 'success';
  ...
  async function ensureReceiptsTable() {
    try {
      ...
      statusMessage = 'No se pudo acceder a la base de datos. Intenta navegar al Dashboard y volver.';
      statusType = 'error';
      return false;
    }
  }
  ...
  onMount(async () => {
    ...
    try {
      ...
      statusMessage = 'No se pudo inicializar la base de datos. Intenta navegar al Dashboard y volver.';
      statusType = 'error';
    }
    ...
    <div class="actions-container">
      ...
      {:else}
        ...
        <div class="secondary-actions">
          <button class="action-button upload-button" on:click={() => fileInput.click()} aria-label="Subir imagen desde galería">
            <Icon name="upload" />
          </button>
          <input 
            type="file" 
          ...
          <button class="action-button data-button" on:click={goToData} aria-label="Ver datos capturados">
            <Icon name="database" />
          </button>
          
          {#if showFullscreenButton}
            <button class="action-button fullscreen-button" on:click={toggleFullscreen} aria-label="Pantalla completa">
              <Icon name="maximize" />
            </button>
          {/if}
        </div>

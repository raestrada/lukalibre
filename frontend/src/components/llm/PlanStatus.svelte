<script lang="ts">
  ...
  $: {
    if (plan) {
      ...
      console.log('¿Se cumple la condición de mostrar plan dev?:', 
...
{#if plan}
  <div class="plan-status {plan.is_active ? '' : 'inactive'}">
    {#if (plan.developer || plan.is_developer) && plan.dev_plan_active}
      <div class="plan-title" style="color:#1a7f37;">Plan: <b>{plan.plan_name || 'Desarrollador LukaLibre'}</b> <span style="font-size:0.92em; color:#1a7f37;">(dev)</span></div>
    ...
    {:else if (plan.developer || plan.is_developer) && plan.dev_plan_active === false}
      <div class="plan-title" style="color:#888;">Plan: <b>Desarrollador LukaLibre</b> <span style="font-size:0.92em; color:#888;">(dev inactivo)</span></div>
    ...
    {:else}
      ...
      <div class="plan-credits" style="color: {plan.is_active ? '#222' : '#aaa'};">Créditos restantes: <b>{plan.credits}</b></div>
      {#if !plan.is_active}
